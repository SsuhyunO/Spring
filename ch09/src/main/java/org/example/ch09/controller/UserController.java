...
public class UserController {
    ...
    public ResponseEntity<UserDTO> login(@RequestBody UserDTO dto, HttpServletResponse response){
        ...
        cookie2.setPath("/");
        cookie2.setMaxAge(60 * 60 * 24 * 5); // 5
    ...
    public ResponseEntity<UserDTO> view(@PathVariable("userid") String userid){
        log.info(userid);
    ...
    // @ResponseBody
    @PutMapping("/user")
    public ResponseEntity<UserDTO> modify(@RequestBody UserDTO dto){
        log.info(dto);

        UserDTO modifiedUser = service.modify(dto);
    ...
    public ResponseEntity<Map<String, Boolean>> delete(@PathVariable("userid") String userid){
        log.info(userid);

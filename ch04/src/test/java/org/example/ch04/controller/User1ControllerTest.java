...
class User1ControllerTest {
    ...
    void list() throws Exception {
        mockMvc.perform(get("/user1/list"))             // GET:/user1/list 요청
                .andExpect(status().isOk())             // 요청성공(200) 학인
                .andExpect(view().name("/user1/list"))  // View(/user1/list.html)
    ...
    void register() throws Exception{
        ...
        for(int i=1;i<=10000;i++) {
            mockMvc.perform(
                            post("/user1/register")
                                    .param("userid", "test10"+i)
                                    .param("name", "테스트1")
                                    .param("birth", "2000-12-30")
                                    .param("hp", "010-1111-1111")
                                    .param("age", "77")
                    )
                    .andExpect(status().is3xxRedirection())
                    .andExpect(redirectedUrl("/user1/list?register=success"))
                    .andDo(print());
        }
    }
    ...
    void remove() throws Exception{
        for(int i=1;i<=10000;i++) {
            mockMvc.perform(
                    ...
                    .andExpect(status().is3xxRedirection())
                    .andExpect(redirectedUrl("/user1/list?remove=success"))
                    .andDo(print());
        }
    }
}